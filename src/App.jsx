  ...
  return (
    <div className='bg-black h-full text-white m-2'>
      ...
      <div className='flex flex-wrap gap-5 p-6 justify-around overflow-scroll h-[80vh]'>
        {loading ? (
          <h1 className='text-3xl font-bold'>Loading...</h1>) : ( image.map((elem) => (
                                                      <Image_Card key={elem.id} id={elem.id} link={elem.link} author={elem.author}/>
          ))
        )}
      </div>
      <div className="flex page h-20 justify-center items-center gap-10">
        <button 
            ...
            className='bg-yellow-400 cursor-grab active:scale-110 h-10 w-20 font-medium text-2xl rounded-2xl text-black'>Prev</button>
        <h1>Page  {pag}</h1>
        <button 
        onClick={PageIncrement} 
        disabled={pag===199}
            style={{opacity : pag===199 ? 0.5 : 1}}
        className='bg-yellow-400 cursor-grab active:scale-110 h-10 w-20 font-medium text-2xl rounded-2xl text-black'>Next</button>
      </div>
    </div>
  )
}
